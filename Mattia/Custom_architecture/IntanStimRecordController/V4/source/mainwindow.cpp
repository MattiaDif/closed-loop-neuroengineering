...
MainWindow::MainWindow(int sampleRateIndex_, int stimStepIndex)
{
    ...
    //highpassFilterFrequency_CA = 250.0; // update GUI - SB - 05-04-2019
    highpassFilterEnabled = false;
    signalProcessor->setHighpassFilterEnabled(highpassFilterEnabled);
    hardThreshold_CA = 0;   //Hard Threshold (μV)   //update GUI MD 03-09-2021
    refractory_CA = 1; //Refractory Period (ms)    //update GUI MD 07-09-2021
...
//    stimGroupBox->setLayout(stimLayout);
    ...
    yScaleList.append(6400); // SB added this to display all possible values
...
//    yScaleDcAmpList.append(10);
    yScaleDcAmpList.append(50); // SB added this after conversion from DC to filtered
    yScaleDcAmpList.append(100); // SB added this after conversion from DC to filtered
    yScaleDcAmpList.append(200); // SB added this after conversion from DC to filtered
    yScaleDcAmpList.append(500); // SB added this after conversion from DC to filtered
    yScaleDcAmpList.append(1000); // SB added this after conversion from DC to filtered
    yScaleDcAmpList.append(2000); // SB added this after conversion from DC to filtered
    yScaleDcAmpList.append(5000); // SB added this after conversion from DC to filtered
    yScaleDcAmpList.append(6400); // SB added this after conversion from DC to filtered
    ...
    ampComboBox->addItem("Filtered"); // SB modified, it was DC
...
//    sampleRateComboBox->setCurrentIndex(3);
    ...
    QFrame *frameTab6 = new QFrame(); // update GUI - SB - 05-04-2019
...
// start - SB - 05-04-2019
    cst_en_CheckBox = new QCheckBox(tr("Enable"));
    cst_trig_CheckBox = new QCheckBox(tr("Trigger"));

    connect(cst_en_CheckBox, SIGNAL(clicked()), this, SLOT(custom_architecture_Enable()));
    connect(cst_trig_CheckBox, SIGNAL(clicked()), this, SLOT(custom_architecture_trig_Enable()));

    QVBoxLayout *custom_en_Layout = new QVBoxLayout;

    QHBoxLayout *cust_en_Layout1 = new QHBoxLayout;
    cust_en_Layout1->addWidget(cst_en_CheckBox);
    cust_en_Layout1->addWidget(cst_trig_CheckBox);
    cust_en_Layout1->addStretch(1);

    custom_en_Layout->addLayout(cust_en_Layout1);

    QGroupBox *customGroupBox = new QGroupBox(tr("Custom enable"));
    customGroupBox->setLayout(custom_en_Layout);

    /*highpassFilterLineEdit_CA = new QLineEdit(QString::number(highpassFilterFrequency_CA, 'f', 0));
    highpassFilterLineEdit_CA->setValidator(new QDoubleValidator(0.01, 9999.99, 2, this));
    connect(highpassFilterLineEdit_CA, SIGNAL(textChanged(const QString &)),
            this, SLOT(highpassFilterLineEditChanged_CA()));*/


    hardThresholdLineEdit_CA = new QLineEdit(QString::number(hardThreshold_CA, 'f', 0));    // update GUI MD 03-09-2021
    hardThresholdLineEdit_CA->setValidator(new QDoubleValidator(0.01, 9999.99, 2, this));   // update GUI MD 03-09-2021
    connect(hardThresholdLineEdit_CA, SIGNAL(textChanged(const QString &)), // update GUI MD 03-09-2021
            this, SLOT(hardThresholdLineEditChanged_CA())); // update GUI MD 03-09-2021


    refractoryLineEdit_CA = new QLineEdit(QString::number(refractory_CA, 'f', 0));    // update GUI MD 07-09-2021
    refractoryLineEdit_CA->setValidator(new QDoubleValidator(0.01, 9999.99, 2, this));   // update GUI MD 07-09-2021
    connect(refractoryLineEdit_CA, SIGNAL(textChanged(const QString &)), // update GUI MD 07-09-2021
            this, SLOT(refractoryLineEditChanged_CA())); // update GUI MD 07-09-2021


    QVBoxLayout *cust_ParamsLayout =  new QVBoxLayout;
    cust_ParamsLayout->addWidget(customGroupBox);
    //cust_ParamsLayout->addWidget(highpassFilterLineEdit_CA);
    //cust_ParamsLayout->addWidget(new QLabel(tr("Hz")));
    cust_ParamsLayout->addWidget(hardThresholdLineEdit_CA); // update GUI MD 03-09-2021
    cust_ParamsLayout->addWidget(new QLabel(tr("Threshold (μV) - For all channels"))); // update GUI MD 03-09-2021
    cust_ParamsLayout->addWidget(refractoryLineEdit_CA); // update GUI MD 07-09-2021
    cust_ParamsLayout->addWidget(new QLabel(tr("Refractory period (ms) - For all channels"))); // update GUI MD 07-09-2021
    cust_ParamsLayout->addStretch(1);
    cust_ParamsLayout->addStretch(1);

    frameTab6->setLayout(cust_ParamsLayout);
...
// stop update GUI - SB - 05-04-2019
    ...
    tabWidget1->addTab(frameTab6, tr("Custom Architecture")); // - SB - 05-04-2019
...
void MainWindow::createActions()
{
    ...
    //start update GUI MD 13-09-2021

    loadConfigFileAction_CA = new QAction(tr("Load Configuration File"), this);
    connect(loadConfigFileAction_CA, SIGNAL(triggered()),
            this, SLOT(loadConfigFile_CA()));

    //end update GUI MD 13-09-2021
...
void MainWindow::createMenus()
{
    ...
    fileMenu->addAction(loadConfigFileAction_CA); //update GUI MD 13-09-2021
...
void MainWindow::hardThresholdLineEditChanged_CA()
{
    setHardThreshold_CA(hardThresholdLineEdit_CA->text().toDouble());
}


void MainWindow::refractoryLineEditChanged_CA()
{
    setRefractory_CA(refractoryLineEdit_CA->text().toDouble());
}


void MainWindow::setHardThreshold_CA(double th)
{
    hardThreshold_CA = th;

    if (!synthMode) {
        evalBoard->setCA_HardThreshold(th);
    }
}


void MainWindow::setRefractory_CA(double refractory)
{
    refractory_CA = refractory;

    if (!synthMode) {
        evalBoard->setCA_Refractory(refractory);
    }
}

// stop update GUI MD 03-09-2021




// stop update GUI MD 07-09-2021
...
void MainWindow::custom_architecture_Enable()
{
    if (!synthMode) {
        evalBoard->custom_architecture_Enable(cst_en_CheckBox->isChecked());
    }
}

void MainWindow::custom_architecture_trig_Enable()
{
    if (!synthMode) {
        evalBoard->custom_architecture_trig_Enable(cst_trig_CheckBox->isChecked());
    }
}
...
{
    unsigned int i;
    double conf_data_double[3];
    unsigned int conf_data[3];

    QString loadConfigFileFileName_CA;
    loadConfigFileFileName_CA = QFileDialog::getOpenFileName(this,
                                                        tr("Select Configration File"), ".txt");

    if (loadConfigFileFileName_CA.isEmpty()) {
        cout << "Loading Configuration File EMPTY" << endl;
        return;
    }

    QFile ConfigFile(loadConfigFileFileName_CA);
    if (!ConfigFile.open(QIODevice::ReadOnly)) {
        cerr << "CAN'T OPEN Configuration File " <<
                loadConfigFileFileName_CA.toStdString() << endl;
        return;
    }
    ...
    while (!ConfigFile.atEnd()) {
        ...
        conf_data_double[i] = file.toDouble(&ok);     // hex == 255, ok == true
    ...
    for (i=0; i<3; ++i) {
        cout << "config file - reading check: " << conf_data[i] << endl;
    }
