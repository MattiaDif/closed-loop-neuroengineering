...
module process_and_retrieve
  ...
  reg [15:0] Memory_out1;  // uint16
  wire input_state_AC_TRUE;
  reg  [0:2] Memory3_reg;  // ufix1 [3]
  wire [0:2] Memory3_reg_next;  // ufix1 [3]
  wire input_state_AC_TRUE_3;
  reg signed [31:0] Memory6_out1 [0:1];  // sfix32_En9 [2]
  ...
  reg [8:0] Memory4_reg [0:1];  // ufix9 [2]
  wire [8:0] Memory4_reg_next [0:1];  // ufix9 [2]
  ...
  assign input_state_AC_TRUE_3 = Memory3_reg[2];
  assign Memory3_reg_next[0] = input_state_AC_TRUE;
  assign Memory3_reg_next[1] = Memory3_reg[0];
  assign Memory3_reg_next[2] = Memory3_reg[1];
  ...
  always @(posedge clk or posedge reset)
    begin : Memory6_process
      if (reset == 1'b1) begin
        Memory6_out1[0] <= 32'sb00000000000000000000000000000000;
        Memory6_out1[1] <= 32'sb00000000000000000000000000000000;
      end
      else begin
        if (enb) begin
          Memory6_out1[0] <= previous_sample[0];
          Memory6_out1[1] <= previous_sample[1];
        end
      end
    end
  ...
  digital_filter u_digital_filter (.clk(clk),
                                   .reset(reset),
                                   .enb(enb),
                                   .current_sample(Memory_out1),  // uint16
                                   .previous_sample_0(Memory6_out1[0]),  // sfix32_En9
                                   .previous_sample_1(Memory6_out1[1]),  // sfix32_En9
  ...
  always @(posedge clk or posedge reset)
    begin : Memory4_process
      if (reset == 1'b1) begin
        Memory4_reg[0] <= 9'b000000000;
        Memory4_reg[1] <= 9'b000000000;
      end
      else begin
        if (enb) begin
          Memory4_reg[0] <= Memory4_reg_next[0];
          Memory4_reg[1] <= Memory4_reg_next[1];
        end
      end
    end

  assign write_porcessing_addr_read_from_processing_addr_2 = Memory4_reg[1];
  assign Memory4_reg_next[0] = read_from_processing_addr;
  assign Memory4_reg_next[1] = Memory4_reg[0];
