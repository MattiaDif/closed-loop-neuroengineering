...
timestamps_per_sim=cell(numSims,1);
for curr_sim = 1:numSims
        timestamps_per_sim{curr_sim,1}=find(ts(curr_sim).Data);
end
ts_ground_truth=find(spike_train(2,:));
matched_spikes=zeros(numSims,1);
for curr_sim = 1:numSims
    ts_curr_sim=timestamps_per_sim{curr_sim,1};
    ...
    for curr_spike=1:n_spikes_curr_sim
        if min(abs(ts_curr_sim(curr_spike)-ts_ground_truth)) <= tol
            matched_spikes(curr_sim,1)=matched_spikes(curr_sim,1)+1;
        end
    end
end
...
%% get spikes above refractory period
ts=timeseries;
n_above_refractory=zeros(numSims,1);
for curr_sim = 1:numSims
        simOut = out(curr_sim);
        ts(curr_sim,:) = simOut.logsout.get('spike_above_refractory').Values;
        n_above_refractory(curr_sim,1)=sum(ts(curr_sim,:).Data);
end
figure
plot(th_sweep,n_above_refractory)
hold on
plot([th_sweep(1) th_sweep(end)],[n_ground_truth n_ground_truth],'-.')
xlabel('threshold')
ylabel('# of matched spikes')
