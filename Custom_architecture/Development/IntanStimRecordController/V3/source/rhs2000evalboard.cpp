...
void Rhs2000EvalBoard::setCA_HardThreshold(double th)
{
    ...
    unsigned int toFPGA[2];
    unsigned int pipe_in;
    unsigned char data_out[sizeof(data_in)];
    ...
    else if (uint16_th > 65535) {
        uint16_th = 65535;
    }

    cout << "uint16_th: " << uint16_th << endl;
    ...
    for (i = 0; i < 2; ++i) {
        data_in[2 * 0] = (unsigned char)((toFPGA[i] & 0x00ff) >> 0);    //LSB
        data_in[2 * 0 + 1] = (unsigned char)((toFPGA[i] & 0xff00) >> 8);    //MSB


            dev->WriteToPipeIn(PipeInCustomArch, sizeof(data_in), data_in);

            pipe_in = (unsigned int)((unsigned char)(data_in[2 * 0 + 1]) << 8 |    //input buffer check
                                            (unsigned char)(data_in[2 * 0]) << 0);
            cout << "pipe in: " << pipe_in << endl;
            ...
            cout << "pipe out: " << pipe_out << endl;
    }
}

void Rhs2000EvalBoard::setCA_Refractory(double refractory)
{
    lock_guard<mutex> lockOk(okMutex);

    unsigned int i;
    unsigned int sample_refractory = (refractory/1000)*getSampleRate(); //conversion from ms to samples

    if (sample_refractory < 0) {
        sample_refractory = 0;
    }

    cout << "refractory " << sample_refractory << endl;

    for (i = 0; i < 1; ++i) {
        data_in[2 * i + 2] = (unsigned char)((sample_refractory & 0x00ff) >> 0);    //LSB
        data_in[2 * i + 3] = (unsigned char)((sample_refractory & 0xff00) >> 8);    //MSB
    }
