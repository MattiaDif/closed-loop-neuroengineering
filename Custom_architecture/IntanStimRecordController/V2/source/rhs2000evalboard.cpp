...
// for example.  This is useful when using the low-latency FPGA thresholds to detect spikes
// and produce digital pulses on the TTL outputs, for example.
/*void Rhs2000EvalBoard::setCA_HighpassFilter(double cutoff)
{
    lock_guard<mutex> lockOk(okMutex);
    ...
    unsigned int i;
    vector<unsigned int> instruction_value_List;
    instruction_value_List.push_back(0x0200);               //16 bits 0x0200
    instruction_value_List.push_back(filterCoefficient);    //16 bits

    for (i = 0; i < 1; ++i) {
        instruction[2 * i] = (unsigned char)((instruction_value_List[i] & 0x00ff) >> 0); // 8 bits
        instruction[2 * i + 1] = (unsigned char)((instruction_value_List[i] & 0xff00) >> 8); // 8 bits
        instruction_value[2 * i] = (unsigned char)((instruction_value_List[i+1] & 0x00ff) >> 0); // 8 bits
        instruction_value[2 * i + 1] = (unsigned char)((instruction_value_List[i+1] & 0xff00) >> 8);// 8 bits
    }
    cout << "instruction_value_List " << instruction_value_List[0] << endl<< instruction_value_List[1] << endl;

    cout << "instruction " << (int)instruction[0] << endl<< (int)instruction[1] << endl;

    cout << "instruction_value " << (int)instruction_value[0] << endl<< (int)instruction_value[1] << endl;


    dev->WriteToPipeIn(PipeInCustomArch, 2, instruction);   // update GUI - SB - 05-04-2019
    dev->WriteToPipeIn(PipeInCustomArch, 2 , instruction_value);    //update GUI - SB - 05-04-2019

//    cout << "filter coeff6 " << filterCoefficient << endl;
//    dev->WriteToPipeIn(PipeInCustomArch, 2, instruction);
//    for (i=0; i<100; ++i){
//        unsigned char result[2];
//        dev->ReadFromPipeOut(PipeOutData_CA, 2,result);
//        cout << "result " << (int)result[0]  << endl << (int)result[1] << endl;
//    }
...
void Rhs2000EvalBoard::setCA_HardThreshold(double th)
{
    ...
    int i;
    unsigned int uint16_th = (th/0.195)+32768 + 0.5;   //conversion from μV to uint16 according to Intan ADC
                                                       //+0.5 for floor rounding mode
    ...
    for (i = 0; i < sizeof(data_in)/2; ++i) {
        data_in[2 * i] = (unsigned char)((uint16_th & 0x00ff) >> 0);    //LSB
        data_in[2 * i + 1] = (unsigned char)((uint16_th & 0xff00) >> 8);    //MSB
    }

    /*for (i = 0; i < sizeof(data_in); i++) {
        data_in[2 * i] = (unsigned char)((instruction_value_List[i] & 0x00ff) >> 0); // 8 bits
        instruction[2 * i + 1] = (unsigned char)((instruction_value_List[i] & 0xff00) >> 8); // 8 bits
        instruction_value[2 * i] = (unsigned char)((instruction_value_List[i+1] & 0x00ff) >> 0); // 8 bits
        instruction_value[2 * i + 1] = (unsigned char)((instruction_value_List[i+1] & 0xff00) >> 8);// 8 bits
    }*/
...
void Rhs2000EvalBoard::custom_architecture_Enable(bool cst_en)
{
    ...
    dev->SetWireInValue(WireInCST_en, (cst_en ?  0b0000000000000001 :  0b0000000000000000),  0b0000000000000001);
    dev->UpdateWireIns();
    cout << "  enable(1)/disable(0) cst en " << cst_en << endl;
}

void Rhs2000EvalBoard::custom_architecture_trig_Enable(bool cst_trig_en)
{
    lock_guard<mutex> lockOk(okMutex);

    dev->SetWireInValue(WireInCST_en, (cst_trig_en ? 0b0000000000000100 : 0b0000000000000000), 0b0000000000000100);
    dev->UpdateWireIns();
